...
- **CQRS через MediatR** — эндпоинты (`Endpoints/`) тонкие: парсят запрос,
  ...
  через `ISender`, вся бизнес-логика — в `Features/<Entity>/*Command.cs` /
  `*Query.cs` (record + handler в одном файле). Авторизация намеренно осталась
  в эндпоинтах, а не в pipeline behavior — усложнение не оправдано для двух
  проверок роли. Не всё продиктовано этим паттерном: `POST /api/auth/logout` и
  `GET /api/auth/me` — чистые cookie-операции без бизнес-логики, остались
  простым кодом эндпоинта.
- **AutoMapper** — для скалярных полей `Quiz`/`QuizAttempt`/`QuestionResponse`
  ...
  `QuestionMapper` — discriminated-union fan-out плохо ложится на AutoMapper,
  переписывать его через 18 профилей было бы больше кода, чем сам маппер.
